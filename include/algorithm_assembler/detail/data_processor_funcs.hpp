...
namespace algorithm_assembler::detail
{
	...
	template<class F, typename Input,  typename... Inputs,
		typename = std::enable_if_t<!utils::is_tuple_v<std::remove_reference_t<Input>>>
	>
	inline auto process_though_functor(F& f, Input&& in,  Inputs&&... ins) -> typename F::Output_type
	{
		return f(std::forward<Input>(in), std::forward<Inputs>(ins)...);
	}
